            ...
            <div className="card-details-center">
              ...
              <div className="card-details-center-price">
                {this.props.type === 'sale-product'
                  ? <del>{(parseInt(this.props.price) * 1.5).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}₫</del>
                  : null
                }
                <span>{this.props.price.replace(/\B(?=(\d{3})+(?!\d))/g, ".")}₫</span>
              </div>
              <div className="card-details-center-button">
                <Link to={`/products/${this.props.path}`}>
                  <Button text='Xem thêm' onClick={() => this.props.handleChangePath()} />
                </Link>
              </div>
            </div>
          </div>
