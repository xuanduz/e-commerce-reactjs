...
const items = localStorage.getItem('CART-ITEMS') !== null ?
...
const rootReducer = (state = initState, action) => {
  ...
  switch (action.type) {
    case 'ADD_TO_CART':
      let existItem = state.listProduct.findIndex((item) =>
    ...
    case 'DECREASE':
      let existDecrease = state.listProduct.findIndex((item) =>
        item.slug === action.payload.slug &&
        item.color === action.payload.color &&
        item.size === action.payload.size
      )
        ...
        listProduct: [
          ...state.listProduct.map((item, index) => {
            ...
            if (index === existDecrease && item.amount >= 1) {
              decreaseAmount.amount = state.listProduct[existDecrease].amount - 1
    ...
    case 'INCREASE':
      let existIncrease = state.listProduct.findIndex((item) =>
        item.slug === action.payload.slug &&
        item.color === action.payload.color &&
        item.size === action.payload.size
      )
        ...
        listProduct: [
          ...state.listProduct.map((item, index) => {
            ...
            if (index === existIncrease) {
              increaseAmount.amount = state.listProduct[existIncrease].amount + 1
    ...
    case 'ON_DELETE':
      let existDelete = state.listProduct.findIndex((item) =>
        item.slug === action.payload.slug &&
        item.color === action.payload.color &&
        item.size === action.payload.size
      )
